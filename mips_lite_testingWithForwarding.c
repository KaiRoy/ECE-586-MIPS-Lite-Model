...
int timing_sim(struct instruction instr, struct Memory *memory, struct Registers *registers)
{
	new_instruction = instr; //creating a temp value for instruction

	//Shift instructions through pipeline. Order is important here
	//This is shifting through the pipeline, think of it as the diagram
	//variables below come from the global variables
	...
	clk_cnt++; //increase the clock by 1
    ...
    hazard_detection_with_FWD:
		...
		else if(instruction_type(ID_stage.code) == RTYPE )
		{
			if(1) //check exe stage
			{
				...
				else if(instruction_type(EXE_stage.code) == RTYPE)
				{
					...
					//if the rd == rs or rd == rs, we will need to forward the information thus the flag gets activated
		...
		else //instruction_type(ID_stage) == ITYPE
		{
			...
			if(1) //check mem stage
			{
     		...
     		stalls_with_forwarding = 0;
				/*if(ID_EXE_flag == 1 && EXE_MEM_flag_forwarding == 0)
		{
		...
		else if(ID_EXE_flag_forwarding == 0 && EXE_MEM_flag_forwarding == 1)
		{
			stalls_with_forwarding += 1;
		}
		else //no hazard
